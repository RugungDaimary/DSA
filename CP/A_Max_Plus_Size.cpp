...
// int fun(int idx, int maxi, int sz, vi &a)
// {
//     int n = a.size();
//     if (idx >= n)
//     {
//         return sz + maxi;
//     }
//     if (dp[idx][maxi + 1][sz] != -1)
//     {
//         return dp[idx][maxi + 1][sz];
//     }
//     int take = fun(idx + 2, max(maxi, a[idx]), sz + 1, a);
//     int not_take = fun(idx + 1, maxi, sz, a);
//     return dp[idx][maxi + 1][sz] = max(take, not_take);
// }
...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
        cin >> a[i];
    // dp.resize(n, vector<vector<int>>(1002, vector<int>(n + 1, -1)));
    // memset(dp,-1,sizeof(dp));
    // int ans = fun(0, -1, 0, a);
    // cout<<ans<<endl;
    // // cout << fun(0, -1, 0, a) << endl;
    int cnt1=0;
    int maxi1=0;
    int cnt2=0;
    int maxi2=0;
    ...
    cout<<max(cnt1+maxi1,cnt2+maxi2)<<endl;
